...
baltcity_income %>%
  select(Census, Neighborhood, x2010, x2020) %>%
    mutate(Diff_Income = (x2020-x2010)) %>% 
    mutate(Diff_Pct_2020 = (x2020-x2010)/x2010*100,2)
...
So we know who the winners since the descending 
by `Diff_Pct_2020` shows us Census tracts by the highest percentage first. 
...
```{r}
md_cities<- rio::import("city_md_income.csv")
```
...
```{r}
 md_cities %>%
  select(place,median_inc_2010,median_inc_2020) %>%
    ...
    mutate(Diff_Pct_2020 = (median_inc_2020-median_inc_2010)/median_inc_2010*100)
```
...
#md_cities %>%
 # select(place,median_inc_2010,median_inc_2020) %>%
 # summarise
 #   (x2020_median - x2010_median) 
#(max_2020 = max(x2020, na.rm=TRUE)
  
md_cities <- md_cities %>% 
  select(place, median_inc_2010, median_inc_2020) %>% 
  mutate(absolute_change = median_inc_2020 - median_inc_2010) %>% 
  mutate(percentage_change = ((median_inc_2020 - median_inc_2010)/median_inc_2010)*100)
...
```{r}
top_25_places <- md_cities %>% 
  select(place, percentage_change) %>% 
...
```{r}
least_25_places <- md_cities %>% 
  select(place, percentage_change) %>% 
  arrange(percentage_change) %>% 
  head(25)

view(least_25_places)
```
...
```{r} 
# I would use the "median" function on the percentage change column. For the tables, I would create a new dataset from the main dataset - which is md_cities - select the percentage_change and the neighborhoods, arrange the percentage change in both ascending and descending order and use the head function to show the top 10 for each. 
``` 
