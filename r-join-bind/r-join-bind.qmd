...
```{R}
#Joining dataframes like this may be difficult because there must be matches on one or more columns,however, they may have similar columns.  If we join these datsets to make a relational database, I think ta table would appear with columns that display population of those in Kent county name of the employers over 1000 which we can use to specify a common key for R use to use to match the data elements.
