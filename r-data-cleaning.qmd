...
```{r}
#In order for the group and summarization works properly, we could write code so R  will understand to ignore NA values. This can e done by by adding na.rm=TRUE when totaling the data or usingt the SUM. I addition, we could use the the glimpse function, and we can update the data from by sorting so R does not store it as a character. this can be done by mutating the state column and making the data type numeric. 
``` 
