...
corrected_pop_2020 <- rio::import("~/Documents/GitHub/data-class-fall-2022/final project 2.5/new corrected pop.xls")
...
#Graphic 1
ggplot(suspensions_expulsions, aes(rate_of_suspension_and_expulsion, disability_status_sex_and_race_ethnicity, fill = rate_of_suspension_and_expulsion)) +
geom_col(position = "dodge")+
labs(
  title = "Expulsions and Suspensions for Disabled Students comapred to All")
...
# 41 is the row for ALL_STUDENTS
...
#3:  Before understanding the suspenions and expulsions minorities face in comparison to white students, I first had to obtain the entire population of students and innerjoin the tables beneath which would create a one table with type of student, suspensions, old and new population etc. 
...
```{r}
#clean the corporal punishment:
#do this for expulsion under 0 tolerance and educational services 
...
ler_race <- updated_school_pop %>% 
  select(Type,all_suspensions,all_expulsions,expul) %>% 
  filter((Type == "All_Black") | (Type == "All_Hispanic") | (Type == "All_White")) %>% 
...
# race_corp %>% 
#   ggplot() +
#   geom_bar(aes(x=reorder(disability_status_sex_and_race_ethnicity, corporal_punishment), weight=corporal_punishment), fill = "blue") +
#   coord_flip() + 
#   labs(
...
```{r}
population <- rio::import("/Volumes/GoogleDrive/My Drive/Data Class Fall 2022/Final Project/tabn225.90.xls")

```



Inner join Education_Stats_CLEAN to population.
1) Create a new table, using a filter, make a smaller version of  Education_Stats_CLEAN that matches the similar values in population.
2) then you can join the two.
